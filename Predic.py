...
def make_model(data_stud: pd.DataFrame):
    ...
    :param data_stud: Данные для модели
    :return:
    """
...
if __name__ == '__main__':
    ...
    # Предсказывание отчисленных из полученных данных

    # Загрузка модели с диска
    filename = 'clf_model'
    clf = pickle.load(open(filename, 'rb'))
    # Предсказание
    testX = np.array(data[['ball1', 'ex_srball']][0:])
    proids = np.array(data[['proid']][0:])
    count0 = 0
    count1 = 0
    print(
'''id студента   Ср. оценка     Ср. балл
            в университете    на ЕГЭ]  Отчислен ли''')
    for i in range(len(testX)):
        pred = clf.predict([testX[i]])
        # Печатать тех, кто возможно будет отчислен
        if pred == 0:
            # print('id студента: ', *list(proids)[i], testX[i],  'Возможно будет отчислен')

            print(f' {list(proids)[i][0]:>8} {list(testX[i])[0]:^18.3f} { list(testX[i])[1]:>8.3f}   Возможно будет отчислен')
        ...
        if clf.predict([testX[i]]) == 0:
            ...
            count1 += 1
    print('\nСтудентов в зоне риска отчисления: ', count0)
    print('Студентов остальных: ', count1)
